...
The relationships between these elements remain implicit. A share certificate describes an ownership interest, but it does not link programmatically to a shareholder record, a share class definition, or a capitalization table. These aggregate relationships (who owns what percentage, how ownership has changed over time) exist in the minds of lawyers and administrators, or in separate spreadsheets maintained alongside the documents, but not in the documents themselves. Extracting the corporate structure from a minute book requires reading and interpretation, not querying.
